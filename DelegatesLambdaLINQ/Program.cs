...
namespace DelegatesLambdaLINQ
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            //var r1 = products.Where(p => p.Category.Tier == 1 && p.Price < 900); === Modo de escrita usando apenas o LINQ
            var r1 = // LINQ com notação(modo de escrita) similar ao SQL, no exemplo abaixo
                from p in products
                where p.Category.Id == 1 && p.Price < 900
                select p;
            ...
            Console.WriteLine("First or Default test1 = {0}", r6);
            var r7 = products.Where(p => p.Price > 3000).FirstOrDefault();
            Console.WriteLine("First or Default test2 = {0}", r7);
            Console.WriteLine();

            var r8 = products.Where(p => p.Id == 3).SingleOrDefault();
            Console.WriteLine("Single Or Default test1 = {0}", r8);
            var r9 = products.Where(p => p.Id == 30).SingleOrDefault();
            Console.WriteLine("Single Or Default test2 = {0}", r9);
            Console.WriteLine();

            var r10 = products.Max(p => p.Price);
            Console.WriteLine("MAX PRICE: {0}", r10);

            var r11 = products.Min(p => p.Price);
            Console.WriteLine("MIN PRICE: {0}", r11);

            var r12 = products.Where(p => p.Category.Id == 1).Sum(p => p.Price);
            Console.WriteLine("Category 1 sum prices = {0}", r12);

            var r13 = products.Where(p => p.Category.Id == 1).Average(p => p.Price);
            Console.WriteLine("Category 1 Average prices = {0}", r13);

            var r14 = products.Where(p => p.Category.Id == 5).Select(p => p.Price).DefaultIfEmpty(0).Average();
            Console.WriteLine("Category 5 Averege prices = {0}", r14);

            var r15 = products.Where(p => p.Category.Id == 1).Select(p => p.Price).Aggregate(0.0, (x,y) => x + y);
            Console.WriteLine("Category 1 aggregate sum: {0}", r15);
            Console.WriteLine();
